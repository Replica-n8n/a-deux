...
/* Le parcours de Love Money. Les écrans ne s'atteignent pas tous par un
   bouton `[data-goto]` : la fiche d'un prêt et la page d'une personne
   s'ouvrent en touchant une ligne de liste. Sans plan explicite, l'audit ne
   les visiterait jamais.

   La première étape SÈME deux prêts si le stockage est vide : sur une app
   sans donnée il n'y a ni liste, ni jauge, ni bandeau, et un « rien à
   signaler » ne signifierait rien.

       await a.audit({ ecrans: a.PLAN_LOVE_MONEY, format: 'texte' });          */
export const PLAN_LOVE_MONEY = [
  { nom: 'accueil', attendu: 'screen-accueil', aller: async () => {
      /* Un audit précédent a pu laisser l'app sur un autre écran. On rentre
         en consommant les entrées d'historique, comme un utilisateur. */
      for (let i = 0; i < 6; i++) {
        const vue = document.querySelector('.screen:not([hidden])');
        if (!vue || vue.id === 'screen-accueil') break;
        const retour = vue.querySelector('[data-back]');
        if (!retour) break;
        retour.click();
        await attendre(160);
      }
      if (window.STORE && !window.STORE.prets().length) {
        const jour = new Date();
        const iso = d => d.getFullYear() + '-' +
          String(d.getMonth() + 1).padStart(2, '0') + '-' +
          String(d.getDate()).padStart(2, '0');
        const bientot = new Date(jour); bientot.setFullYear(jour.getFullYear() - 1); bientot.setDate(jour.getDate() + 12);
        const loin = new Date(jour); loin.setMonth(jour.getMonth() - 2);
        window.STORE.enregistrer({ personne: 'Jean-Guy', montant: 8000, taux: 0, debut: iso(bientot) });
        window.STORE.enregistrer({ personne: 'Marc Tremblay', montant: 15000, taux: 6, debut: iso(loin) });
        window.STORE.enregistrer({ personne: 'Marc Tremblay', montant: 4500, taux: 7, debut: iso(loin) });
  ...
  { nom: 'réglages', attendu: 'screen-reglages',
    aller: () => document.querySelector('[data-goto="reglages"]').click() },

  { nom: 'nouveau prêt', attendu: 'screen-nouveau', aller: async () => {
      document.querySelector('#screen-reglages [data-back]').click();
      await attendre(200);                       // history.back() est asynchrone
      document.querySelector('#btn-ajouter').click();
    } },

  { nom: 'fiche du prêt', attendu: 'screen-pret', aller: async () => {
      document.querySelector('#screen-nouveau [data-back]').click();
      await attendre(200);
      document.querySelector('#echeances .pret').click();
    } },

  { nom: 'page d une personne', attendu: 'screen-personne', aller: async () => {
      document.querySelector('#screen-pret [data-back]').click();
      await attendre(200);
      document.querySelector('#legende .lg').click();
    } }
];

/* Le parcours d'À deux. L'écran de profil ne s'atteint pas par un bouton
   `[data-goto]` : on ouvre une carte de l'accueil, qui est un bouton
   `[data-profil]`. Sans plan explicite, l'audit ne le visiterait jamais.

   La première étape REVIENT à l'accueil en consommant l'historique, puis
   CHOISIT deux langages si rien n'est encore choisi : sur une app vierge les
   cartes sont en creux, et un « rien à signaler » ne signifierait rien.

       await a.audit({ ecrans: a.PLAN_A_DEUX, format: 'texte' });             */
export const PLAN_A_DEUX = [
  { nom: 'accueil', attendu: 'screen-accueil', aller: async () => {
      for (let i = 0; i < 6; i++) {
        const vue = document.querySelector('.screen:not([hidden])');
        if (!vue || vue.id === 'screen-accueil') break;
  ...
  { nom: 'profil, le mien', attendu: 'screen-profil',
    aller: () => document.querySelector('#carte-moi').click() },

  { nom: 'profil, le sien', attendu: 'screen-profil', aller: async () => {
      document.querySelector('#screen-profil [data-back]').click();
      await attendre(200);                       // history.back() est asynchrone
      document.querySelector('#carte-autre').click();
    } },

  { nom: 'réglages', attendu: 'screen-reglages', aller: async () => {
      document.querySelector('#screen-profil [data-back]').click();
      await attendre(200);
      document.querySelector('[data-goto="reglages"]').click();
    } }
